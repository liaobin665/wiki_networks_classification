...
#只包含度的信息
total_pd = pd.read_csv("../data/embedding_file/sdne_wiki_embeding_label_extinfo.csv",
                       usecols=['node_id','label','degree','in_degree','out_degree'])
...
for model in classifiers:
    ...
    ans = classification_report(y_test, y_score, digits=4)
    print(ans)
...
cvResDf = pd.DataFrame({'accuracy': accuracy,
                        ...
                        'f1_score':F1_score,
                        'algorithm': ['SVm']})
#cvResDf.to_csv("../data/result/ext_baseline_svm.csv")
